...
with open(base_dir + 'label.csv', 'w', newline='') as csvfile:
    fieldnames = ['index', 'mid', 'display_name']
    writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
    writer.writeheader()

    for i in range(0, len(label.keys())):
        index = i
        mid = '/m/07rwj' + str(index).zfill(2)
        display_name = label[list(label.keys())[i]]
...
for fold in [1,2,3,4,5]:
    ...
    train_wav_list = []
    eval_wav_list = []
    weight_list = [0] * len(label)
    for i in range(0, len(audio16k_list)):
        cur_label = label[audio16k_list[i].split('.')[0]]
    ...
    with open('./data/datafiles/train_data_'+ str(fold) +'.json', 'w') as f:
        json.dump({'data': train_wav_list}, f, indent=1)

    with open('./data/datafiles/eval_data_'+ str(fold) +'.json', 'w') as f:
        json.dump({'data': eval_wav_list}, f, indent=1)
        
    np.savetxt('./data/datafiles/train_data_'+ str(fold) + '_weight.csv', train_weight_list, delimiter=',')
